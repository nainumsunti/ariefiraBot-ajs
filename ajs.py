...
ks = LineClient()
#ks = LineClient(authToken='MASUKAN TOKENMU DISINI')
ks.log("Auth Token : " + str(ks.authToken))
channel3 = LineChannel(ks)
ks.log("Channel Access Token : " + str(channel3.channelAccessToken))
...
responsename4 = ks.getProfile().displayName
        ...
        if op.type == 19:
            ...
            if admin in op.param3:
                ...
                if op.param3 in staff:
                    ...
                    try:
                        cl.kickoutFromGroup(op.param1,[op.param2])
                        cl.findAndAddContactsByMid(op.param3,[admin])
                        cl.inviteIntoGroup(op.param3,[admin])
                    except:
                        try:
                            ki.kickoutFromGroup(op.param1,[op.param2])
                            ki.findAndAddContactsByMid(op.param3,[admin])
                            ki.inviteIntoGroup(op.param3,[admin])
                        except:
                            try:
                                kk.kickoutFromGroup(op.param1,[op.param2])
                                kk.findAndAddContactsByMid(op.param3,[admin])
                                kk.inviteIntoGroup(op.param3,[admin])
                            except:
                                pass
...
#===========BOT UPDATE============#
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                ks.sendMessage(msg.to,responsename4)

                        elif cmd == "invitebot":
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                try:
                                    anggota = [Dmid,Cmid,Bmid,Amid]
                          ...
                          if wait["selfbot"] == True:
                              if settings["autoJoinTicket"] == True:
                                 ...
                                 for ticket_id in n_links:
                                     ...
                                     group4 = ks.findGroupByTicket(ticket_id)
                                     ks.acceptGroupInvitationByTicket(group3.id,ticket_id)
